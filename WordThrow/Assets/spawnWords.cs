...
public class spawnWords : MonoBehaviour {
	...
	void Update () {
		...
		if (countdown <= 0) {
			...
			Vector3 offset = new Vector3(Random.Range(-bc.size.x / 2, bc.size.x / 2), Random.Range(-bc.size.y / 2, bc.size.y / 2), Random.Range(-bc.size.z / 2, bc.size.z / 2));
